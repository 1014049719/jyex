...
#define CPPSQLITE_DB_OP_DISABLE 1001 /*用户禁止数据库操作*/ 
/* end-of-error-codes */
...
// 成员函数
	...
	long 		GetBigIntField(int nField, long  nNullValue=0);  //获取整型字段值
	long 		GetBigIntField(const char* szField, long  nNullValue=0);
...
// 成员函数
    ...
    int ExecDML();                      //执行SQL语句
    void Bind(int nParam, const char* szValue);             //添加文本值。 nParam从1开始。
    void Bind(int nParam, const int nValue);                //添加整型值。 nParam从1开始。
    void Bind(int nParam, const double dwValue);            //添加浮点值。 nParam从1开始。
    void Bind(int nParam, const unsigned char* blobValue, int nLen);            //添加二进制值。 nParam从1开始。
    void BindNull(int nParam);          //添加空值。 nParam从1开始。
...
// 成员函数
	...
	int     Cmd(const char* fmt, const va_list args);  //获取数据集
	...
	long   GetPageSize(const char* szKey);
