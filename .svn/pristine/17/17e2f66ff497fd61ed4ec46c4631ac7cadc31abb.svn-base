...
public:
	...
	byte ProcessByte(byte input)
	{
		if (counter==FBS)
			DoFeedBack();
		byte tm = buffer.GetIndex(counter);
		tm ^= input;
		buffer.SetIndex(counter,tm);
		counter++;
		return tm;
	}
...
//		buffer[counter++] = input;
		byte a = buffer.GetIndex(counter);
		byte b = a^input;
		buffer.SetIndex(counter,input);
		counter++;
		return (b);
	}
...
public:
	...
	byte GetByte()
	{
		...
		counter++;
		return buffer.GetIndex(counter-1);
	}
