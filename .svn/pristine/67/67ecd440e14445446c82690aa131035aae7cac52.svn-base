...
- (void)doneAction:(id)sender
{
    //update by zd 20150203
    if ( [AGIPCGridItem numberOfSelections] > 30 - [[GlobalPictureCounter getGlobalPictureCounterInstance] getPictureCount] ) {
        
        NSString *str = [NSString stringWithFormat:@"选择照片过多，请减少至%i张", 30 - [[GlobalPictureCounter getGlobalPictureCounterInstance] getPictureCount]];
        //UIAlertView *alv = [[UIAlertView alloc] initWithTitle:nil message:@"选择照片过多，请减少至30张。" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] ;
        
        UIAlertView *alv = [[UIAlertView alloc] initWithTitle:nil message:str delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] ;
        [alv show];
        return ;
    }
    
    /*
    if ( [AGIPCGridItem numberOfSelections] > 30 ) {
        UIAlertView *alv = [[UIAlertView alloc] initWithTitle:nil message:@"选择照片过多，请减少至30张。" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] ;
        [alv show];
        return ;
    }*/
