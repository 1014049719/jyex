...
- (void) moveAlert: (UIAlertView *) alertView
{
	...
	UITextField *tf = (UITextField *)[alertView viewWithTag:TEXT_FIELD_TAG];
    if (tf)
    {
...
+ (void) say: (id)formatstring,...
{
	...
	va_end(arglist);
	[ModalAlert ask:statement withCancel:@"确定" withButtons:nil];
	[statement release];
}
...
+ (BOOL) ask: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = ([ModalAlert ask:statement withCancel:nil withButtons:[NSArray arrayWithObjects:@"是", @"否", nil]] == 0);
	[statement release];
	return answer;
}
...
+ (BOOL) confirm: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = [ModalAlert ask:statement withCancel:@"取消" withButtons:[NSArray arrayWithObject:@"确定"]];
	[statement release];
	return	answer;
}
...
{
    NSString *newText = [NSString stringWithFormat:@"%@%@%@", [textField.text substringToIndex:range.location], string, [textField.text substringFromIndex:range.location]];
    return [newText length]<=50;
}

+(NSString *) textQueryWith: (NSString *)question prompt: (NSString *)prompt button1: (NSString *)button1 button2:(NSString *) button2 withInitText:(NSString *)initText
{
	// Create alert
	CFRunLoopRef currentLoop = CFRunLoopGetCurrent();
	ModalAlertDelegate *madelegate = [[ModalAlertDelegate alloc] initWithRunLoop:currentLoop];
...
+ (NSString *) ask: (NSString *) question withTextPrompt: (NSString *) prompt withInitText:(NSString *) initText
{
	return [ModalAlert textQueryWith:question prompt:prompt button1:@"取消" button2:@"确定" withInitText:initText];
}
@end
