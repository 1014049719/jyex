...
/*****************************************************************************************
   ...
   NOTES:			Provides an interface to the CMD5Checksum class. Any data that can
                ...
                function. Typically, CString and TCHAR arrays will be checksumed,
...
*****************************************************************************************/
string CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
    //entry invariants
...
/*****************************************************************************************
   ...
   ARGUMENTS:		DWORD* Output : the 32 bit (unsigned long) destination array
                BYTE* Input	  : the 8 bit (BYTE  ) source array
...
*****************************************************************************************/
void CMD5Checksum::Transform(BYTE Block[64]) //BYTE  Block[64]
{
...
string CMD5Checksum::Final()
{
    ...
    for (int i = 0; i < nMD5Size; i++)
    {
        ...
        {
            MYSPRINTF(Str, "0%x", (lpszMD5[i]));
        }
        else
        {
            MYSPRINTF(Str, "%x", (lpszMD5[i]));
        }

        assert(strlen(Str) == 2);  //strlen(Str)/sizeof(TCHAR)
        strMD5 += Str;
    }

    assert(strMD5.length() == 32);  //strMD5.GetLength()
    return strMD5;
}
