...
{
	//unsigned char e_key[8] = {0x44,0x73,0x12,0x4D,0x68,0x59,0xDD,0x4C};
	unsigned char e_key[8] = {0x32,0x30,0x30,0x38,0x30,0x38,0x30,0x32};

	unsigned char buff[512] = {0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38};
	if (argc>1) {
		memset(buff,'\0',sizeof(buff));
		strcpy(buff,argv[1]);

	    deskey((unsigned char*)e_key, EN0);
		int st=0;
		int len=strlen(buff);
		unsigned char *p;
		for (st=0;st<(len+7)/8;st++) {
			p=&buff[0]+8*st;
			des(p,p);
			PRINT("    ",p);
		}
...
//Decrypt
	printf("\n");
    deskey((unsigned char*)e_key, DE1);
	PRINT("En :",buff);
    des(buff, buff);
	PRINT("De :",buff);
}
