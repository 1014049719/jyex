...
: reg(2)
{
	...
	//reg[0] = word(value);
	reg.SetIndex(0,word(value));
	reg.SetIndex(1,sizeof(value)>WORD_SIZE ? word(value>>WORD_BITS) : 0);
...
void Integer::swap(Integer::Sign&a,Integer::Sign&b)
{
	Integer::Sign c = a;
	a = b;
	b = c;
...
{
	int a = (int)reg.GetIndex(0);
	return IsNegative() ? false : (a==0 && WordCount()==0);
}
...
void swap(const word* &A,const word* &B)
{
	const word* C = 0;
	C = A;
	A = B;
...
void swap(unsigned int&NA,unsigned int&NB)
{
	unsigned int C = NA;
	NA = NB;
	NB = C;
...
Integer operator-(const Integer &a, const Integer& b)
{
	Integer diff((word)0, max(a.reg.size, b.reg.size));
	if (a.NotNegative())
	{
		if (b.NotNegative())
...
{
	reg.SetIndex(0,value);
    //	SetWords(reg+1, 0, reg.size-1);
	reg.SetValue(0,1,reg.size-1);
}
...
{
	return (WordCount() == 1) && (reg.GetIndex(0) == 1);
}
...
Integer operator+(const Integer &a, const Integer& b)
{
	Integer sum((word)0, max(a.reg.size, b.reg.size));
	if (a.NotNegative())
	{
		if (b.NotNegative())
			PositiveAdd(sum, a, b);
...
{
	unsigned long value = reg.GetIndex(0);
	value += sizeof(value)>WORD_SIZE ? ((unsigned long)reg.GetIndex(1)<<WORD_BITS) : 0;
...
word operator%(const Integer &dividend, word divisor)
{
	...
	if ((divisor & (divisor-1)) == 0)   // divisor is a power of 2
		remainder = dividend.reg.GetIndex(0) & (divisor-1);
	else
	{
...
void Integer::Randomize(RandomNumberGenerator &rng, unsigned int nbits)
{
	...
	if (nbytes)
	{
		bt = (byte)Crop(buf.GetIndex(0), nbits % 8);
		buf.SetIndex(0,bt);
	}
...
const Integer& IntegerMax(const Integer&A,const Integer&B)
{
	return A>B?A:B;
}
	...
	switch (rnType)
	{
		...
		case PRIME:
			...
			while (1)
			{
				...
				Randomize(rng, min, max);
				if (FirstPrime(*this, IntegerMax(*this+mod*PrimeSearchInterval(max), max), equiv, mod))
					return true;
			}
...
Integer ModularArithmetic::Exponentiate(const Integer &a, const Integer &e) const
{
	if (modulus.IsOdd())
	{
		...
		return AbstractRing::Exponentiate(a, e);
}
