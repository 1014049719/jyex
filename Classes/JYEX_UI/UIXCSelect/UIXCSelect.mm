...
- (void)viewDidLoad
{
    ...
    if ( [msgParam.param1 isEqualToString:@"1"]) {
        nCreatFlag = 0;
        [self onOK:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ( self.msgParam ) {
        if ( self.msgParam.obsv && self.msgParam.callback) {
            [self.msgParam.obsv performSelector:self.msgParam.callback withObject:dicAlbum] ;
        }
    }
...
{
    NSLog( @"取消" ) ;
    if ( nCreatFlag == 0 || [msgParam.param1 isEqualToString:@"1"] ) {
        [self.navigationController popViewControllerAnimated:YES] ;
    }
    else {
        nCreatFlag = 0;
...
- (void)syncCallback_CreateJYEXAlbum:(TBussStatus*)sts
{
    ...
    if( res == 1 ) //创建相册成功
    {
        ...
        if ( self.msgParam ) {
            if ( self.msgParam.obsv && self.msgParam.callback) {
                [self.msgParam.obsv performSelector:self.msgParam.callback withObject:dc] ;
            }
        }
