...
andShouldShowSavedPhotosOnTop:(BOOL)shouldShowSavedPhotosOnTop
{
    ...
    if (self)
    {
        ...
        if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_6_1) {
            ...
            self.navigationBar.tintColor = [UIColor colorWithRed:0x2d/255.0 green:0x9c/255.0 blue:0x2f/255.0 alpha:1.0];
        }
        else {
            self.navigationBar.translucent = NO;
            self.navigationBar.barTintColor = [UIColor colorWithRed:0x2d/255.0 green:0x9c/255.0 blue:0x2f/255.0 alpha:1.0];
...
- (void)didFinishPickingAssets:(NSArray *)selectedAssets
{
    ...
    //2014.4.14 update for jyex
    if ( [self.viewControllers[0] isKindOfClass:[AGIPCAlbumsController class]]) {
        AGIPCAlbumsController __weak *pAGIPCAlbumsController = self.viewControllers[0];
        pAGIPCAlbumsController.imagePickerController = nil; //复原
    }
