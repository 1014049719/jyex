...
const char *g_cGZMonth_FT[] = 
{ 
    "丙寅","丁卯","戊辰","己巳","庚午","辛未","壬申","癸酉","甲戌","乙亥","丙子","丁醜",
    "戊寅","己卯","庚辰","辛巳","壬午","癸未","甲申","乙酉","丙戌","丁亥","戊子","己醜",
    "庚寅","辛卯","壬辰","癸巳","甲午","乙未","丙申","丁酉","戊戌","己亥","庚子","辛醜",
    "壬寅","癸卯","甲辰","乙巳","丙午","丁未","戊申","己酉","庚戌","辛亥","壬子","癸醜",
    "甲寅","乙卯","丙辰","丁巳","戊午","己未","庚申","辛酉","壬戌","癸亥","甲子","乙醜"
};
char **g_cGZMonth = NULL;


const char *g_cGZHour_JT[] = 
{
    "甲子","丙子","戊子","庚子","壬子",
    "乙丑","丁丑","己丑","辛丑","癸丑",
    "丙寅","戊寅","庚寅","壬寅","甲寅",
...
const char **g_cGZHour;


const char *g_cGanZhi_JT[] =
...
const char *g_cTianGan[] = {"甲","乙","丙","丁","戊","己","庚","辛","壬","癸"};

const char *g_cDiZhi_JT[] = {"子","丑","寅","卯","辰","巳","午", "未","申","酉","戌","亥"};
const char *g_cDiZhi_FT[] = {"子","醜","寅","卯","辰","巳","午", "未","申","酉","戌","亥"};
char **g_cDiZhi = NULL;

const char *g_cShengXiao_JT[] = {"鼠","牛","虎","兔","龙","蛇","马","羊","猴","鸡","狗","猪"};
const char *g_cShengXiao_FT[] = {"鼠","牛","虎","兔","龍","蛇","馬","羊","猴","雞","狗","豬"};
char **g_cShengXiao = NULL;


const char *g_cDayName[] = { "*","初一","初二","初三","初四","初五", 
    "初六","初七","初八","初九","初十",
    "十一","十二","十三","十四","十五",
    "十六","十七","十八","十九","二十",
    "廿一","廿二","廿三","廿四","廿五",
...
const char *g_cMonName_JT[] = {"*","正","二","三","四","五","六", "七","八","九","十","十一","腊"};
const char *g_cMonName_FT[] = {"*","正","二","三","四","五","六", "七","八","九","十","十一","臘"};
char **g_cMonName = NULL;

int g_isFT = 0;

void Calendar::InitCalendarLoc(int isFT)
{
    g_isFT = isFT;

    if (isFT)
    {
        g_cGZMonth = (char**)g_cGZMonth_FT;
        g_cGZHour = g_cGZHour_FT;
        g_cGanZhi = (char**)g_cGanZhi_FT;
        g_cDiZhi = (char**)g_cDiZhi_FT;
        g_cShengXiao = (char**)g_cShengXiao_FT;
        g_cMonName = (char**)g_cMonName_FT;
    }
    else
    {
        g_cGZMonth = (char**)g_cGZMonth_JT;
        g_cGZHour = g_cGZHour_JT;
        g_cGanZhi = (char**)g_cGanZhi_JT;
        g_cDiZhi = (char**)g_cDiZhi_JT;
        g_cShengXiao = (char**)g_cShengXiao_JT;
        g_cMonName = (char**)g_cMonName_JT;
    }
...
*/
    ...
    l_lunarinfo.shenxiao = g_cShengXiao[nShuXiang];
    
    int nTianGan = ((l_CalDate.year - 4) % 60) % 10;
    l_lunarinfo.tiangan = g_cTianGan[nTianGan];
    
    int nDiZhi = ((l_CalDate.year - 4) % 60) % 12;
    l_lunarinfo.dizhi = g_cDiZhi[nDiZhi];
    
    l_lunarinfo.isLeepMonth = l_CalDate.isRunYue;
    
    if ( l_CalDate.month < 0 || l_CalDate.month > 12)
	{
        LunarInfo l_lunarinfo_error;
        return l_lunarinfo_error ;
	}
	l_lunarinfo.monthname = g_cMonName[l_CalDate.month];
    
    if (l_CalDate.day<0 || l_CalDate.day>30)
    {
        LunarInfo l_lunarinfo_error;
        return l_lunarinfo_error ;
    }
    
    l_lunarinfo.dayname = g_cDayName[l_CalDate.day];
	return l_lunarinfo;
...
std::string Calendar::GetLlGZDay(DateInfo  date)
{
	...
	return std::string(g_cTianGan[l_i_g-1]) + g_cDiZhi[l_i_z-1];
...
std::string Calendar::GetLlGZMonth(const DateInfo&  date)
{
	...
	for(int i=0;i<10;++i)
	{
		if(l_nlinfo.tiangan==g_cTianGan[i])
		{
			l_i_y = i;
			break;
		}
	...
	for(int i=0;i<13;++i)
	{
		...
		if(l_nlinfo.monthname==g_cMonName[i])
		{
			l_i_m = i;
			break;
		}
    ...
    return g_cGZMonth[l_i_y*5 + l_i_m];
...
std::string Calendar::GetLlGZMonth_GanZhi(const DateInfo&  date)
{
	...
	*****/
	...
	return g_cGanZhi[l_iMonthGzIndx];
}
...
//	const std::string hours[12]  = {"子","丑","寅","卯","辰","巳","午", "未","申","酉","戌","亥"};
    ...
    return g_cGZHour[l_i_hour*12 + l_i_day];
...
{
//	std::string l_s_dizhi[] = IS_FT ? {"子","丑","寅","卯","辰","巳","午", "未","申","酉","戌","亥"} :
//    {"子","醜","寅","卯","辰","巳","午", "未","申","酉","戌","亥"};      

	for(int i =0;i<12;++i)
	{
		if(sDiZhi == g_cDiZhi[i])
		{
			return i;
		}
	}
